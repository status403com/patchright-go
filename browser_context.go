...
import (
	...
	"github.com/status403com/patchright-go/internal/safe"
)
...
func newBrowserContext(parent *channelOwner, objectType string, guid string, initializer map[string]any) *browserContextImpl {
	...
	if bt.browser != nil && bt.browser.browserType != nil {
		if browserType, ok := bt.browser.browserType.(*browserTypeImpl); ok && browserType.patchright != nil {
			browserType.patchright.Selectors.(*selectorsImpl).addContext(bt)
		}
	}
	...
	bt.channel.On("close", func() {
		...
		if bt.browser != nil && bt.browser.browserType != nil {
			if browserType, ok := bt.browser.browserType.(*browserTypeImpl); ok && browserType.patchright != nil {
				browserType.patchright.Selectors.(*selectorsImpl).removeContext(bt)
			}
		}
		bt.onClose()
	})
